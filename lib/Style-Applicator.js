...
function overrides(oldInfo, newInfo) {
	...
	if (oldInfo.specificity.a === newInfo.specificity.a) {
		...
		if (!newInfo.prop.important && oldInfo.prop.important)
			return false;
		...
		if (!oldInfo.prop.important || newInfo.prop.important)
			return false;
	}
...
function cascadeProperties(inlineProperties, elem, rules) {
	...
	var applyProperties = function (props, s, index) {
		props.forEach(function (p) {
			var newInfo = { prop: p, specificity: s, index: index };
			if (propertyValues.hasOwnProperty(p.name)) {
				...
				var oldInfo = propertyValues[p.name];

				if (!overrides(oldInfo, newInfo))
					return;
			}
			propertyValues[p.name] = newInfo;
		});
	};
	...
	_.forOwn(propertyValues, function (appliedInfo, name) {
		...
		dynamicRules[name].forEach(function (dynamicInfo) {
			...
			rule.properties[rule.properties.indexOf(dynamicInfo.prop)] = newProp;
		});
	});
...
function applyStyles(doc, rules) {
	...
	if (!body) {
		...
		promise = applyRecursive(doc.root(), styleSource, doc);
	} else {
		promise = Q.all([
			// Apply any styles for the <html> tag
			applyTo(doc.root()[0], styleSource, doc),
			// Do not touch the <head> tag at all.

			// Apply CSS to the entire <body> tree
			applyRecursive(body, styleSource, doc)
		]);
	}
