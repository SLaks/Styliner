...
function processCachedStyleSheet(pss) {
	...
	util.modifyArraySync(pss.elements, function (elem) {
		if (elem instanceof ParsedStyleSheet.Rule) {
			util.modifyArray(elem.properties, function (prop) {
...
function processDocumentStyleSheet(pss, doc) {
	...
	return util.modifyArrayCopy(pss.elements, function (elem) {
		if (elem instanceof ParsedStyleSheet.Rule) {
			...
			return processPropertyDocument(elem, doc);
		}
		return undefined;
	});
}
...
function processInlineProperty(prop, doc) {
	...
	var props = processPropertyCached(prop, doc.folder, true) || prop;
	if (!(props instanceof Array))
		props = [props];

	return Q.all(
		props.map(function (p) { return processPropertyDocument(p, doc) || p; })
	);
}
