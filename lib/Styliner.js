...
/**
 * Asynchronously parses an HTML document from file path passed as argument and inlines styles as appropriate.
 *
 * @param {String}			filename				The HTML file name to be parsed. filename should be passed with *.html extension
 * @param {String}			filePath				The file path to parse HTML document.
 */
Styliner.prototype.parseFile = function(filename, filePath) {
	var _this = this, path;
	if (arguments.length != 1) {
		path = qfs.join(filePath, filename);
	} else {
		path = filename;
	}
	return  qfs.read(path).then(function(content) {
		return _this.processHTML(content);
	});
}
