		...
		stylesheets.map(function (ss) {
			...
			if (ss.type === "source") {
				...
				return Q.when(ss.text, function (source) {
					var pss = new ParsedStyleSheet(source, htmlPath, self.options);
					return preprocessor.cachedStyleSheet(pss);
				});
			...
			return p.then(function (pss) {
				return preprocessor.documentStyleSheet(pss, doc, relativePath, self.options);
			});
		})
	);
	return Q.spread(
		[
			stylesheetsLoaded,
			preprocessor.htmlDocument(doc, relativePath, self.options)
		],
		function (elements) {
			...
			applyStyles(doc, allRules, self.options);
			return doc.html();
		});
};
