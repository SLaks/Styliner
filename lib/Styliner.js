 ...
 *	url: function(path, type)
			...
			The function is passed the relative path to the file and the source of the URL ("img" or "a" or other HTML tags; URLs from CSS also pass "img")
...
function Styliner(baseDir, options) {
	...
	options.url = options.url
		|| (options.urlPrefix && function (url) { return qfs.join(options.urlPrefix, url).replace('\\', '/'); })
...
Styliner.prototype.getStylesheet = function (path) {
	...
	var promise = qfs.read(fullPath)
		...
		.then(function (source) {
			...
			return preprocessor.cachedStyleSheet(pss);
		});
...
Styliner.prototype.processHTML = function (source, relativePath, stylesheetPaths) {
	...
	doc.folder = qfs.join(this.baseDir, relativePath);
	...
	var htmlPath = qfs.join(doc.folder, '-html-');
		...
		stylesheets.map(function (ss) {
			...
			if (ss.type === "source") {
				...
				return Q.when(ss.text, function (source) {
					var pss = new ParsedStyleSheet(source, htmlPath, doc.options);
					return preprocessor.cachedStyleSheet(pss);
				});
			...
			return p.then(function (pss) {
				return preprocessor.documentStyleSheet(pss, doc);
			});
		})
	);
	return Q.spread(
		...
		function (elements) {
			...
			var allRules = Array.prototype.concat.apply([], elements);
			applyStyles(doc, allRules, doc.options);
			return doc.html();
		});
};
